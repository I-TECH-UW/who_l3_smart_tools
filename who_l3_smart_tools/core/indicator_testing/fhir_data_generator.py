...
def generate_patient_bundle(row, measurement_start, measurement_end):
    ...
    if row['HIV_Positive']:
        ...
        if random.choice([True, False]):
            condition_resource = generate_condition_resource(row, start_date - timedelta(days=10), end_date - timedelta(days=1))
            bundle_resources.append(condition_resource)

    # If HIV_Treatment, add MedicationStatement resource
    if row['HIV_Treatment']:
        medication_resource = generate_medication_statement_resource(row, start_date, end_date)
        ...
        if random.choice([True, False]):
            medication_resource = generate_medication_statement_resource(row, start_date - timedelta(days=10), end_date - timedelta(days=1))
            bundle_resources.append(medication_resource)
    ...
    if row['Deceased']:
        ...
        if random.choice([True, False]):
            patient_resource = add_future_deceased_information(patient_resource, end_date)

    # If Stopped_ART, add an EpisodeOfCare resource with status finished
    if row['Stopped_ART']:
        episode_of_care_resource = generate_episode_of_care_finished_before_measurement(row, end_date)
        ...
        if random.choice([True, False]):
            episode_of_care_resource = generate_episode_of_care_finished_after_measurement(row, end_date)
            bundle_resources.append(episode_of_care_resource)

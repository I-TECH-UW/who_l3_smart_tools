def generate_random_row():
    ...
    # Disaggregation Fields: Get all fields after column with heading `Disaggregation:`
    ...
    hiv_treatment = hiv_positive and choice(
        [True, False]
    )  # Treatment only if HIV positive
...
def compute_counts(df):
    df["Numerator"] = df.apply(
        lambda x: int(
            x["HIV_Positive"]
            and x["HIV_Treatment"]
            and not (x["Deceased"] or x["Stopped_ART"])
        ),
        axis=1,
    )
